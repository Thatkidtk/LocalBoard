...
function mapPost(row: Record<string, unknown>): PostListItem {
  return {
    id: String(row.id),
...
function mapComment(row: Record<string, unknown>): CommentNode {
  return {
    id: String(row.id),
...
export async function getHomePageData(selectedCommunitySlug?: string | null): Promise<HomePageData> {
  ...
  if (selectedCommunitySlug) {
    communityQuery = supabase.from("communities").select("*").eq("slug", selectedCommunitySlug).limit(1);
  ...
  return {
    ...
    nearbyCommunities,
    trendingPosts: (postsResult.data ?? []).map((row) => mapPost(row as unknown as Record<string, unknown>)),
  };
}
...
export async function getCommunityPageData(
  ...
  return {
    ...
    nearbyCommunities,
    posts: (postsRows ?? []).map((row) => mapPost(row as unknown as Record<string, unknown>)),
...
export async function getPostDetail(postId: string): Promise<PostDetail | null> {
  ...
  return {
    ...mapPost(postRow as unknown as Record<string, unknown>),
    comments: buildCommentTree(
      (commentsRows ?? []).map((row) => mapComment(row as unknown as Record<string, unknown>)),
    ),
  };
}
